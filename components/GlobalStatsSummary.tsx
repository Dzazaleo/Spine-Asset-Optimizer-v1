...
import { Trophy, ChevronDown, ChevronUp, Scaling, Edit2, AlertTriangle } from 'lucide-react';
...
interface GlobalStatsSummaryProps {
  ...
  onSort: (key: 'path' | 'originalSize' | 'maxRenderSize' | 'sourceAnimation') => void;
  onAnimationClick?: (animationName: string) => void;
}
...
export const GlobalStatsSummary: React.FC<GlobalStatsSummaryProps> = ({ 
  ...
  const SortableHeader = ({ label, sortKey, align = 'left' }: { label: string, sortKey: 'path' | 'originalSize' | 'maxRenderSize' | 'sourceAnimation', align?: 'left' | 'center' | 'right' }) => {
                    ...
                    <td className="px-4 py-2.5">
                       <div className="flex items-center gap-2 flex-wrap">
                         <button 
                           onClick={(e) => {
                             e.stopPropagation();
                             onAnimationClick?.(stat.sourceAnimation);
                           }}
