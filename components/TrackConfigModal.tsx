...
import { X, Plus, Trash2, GripVertical, Search, PlayCircle, FileText, ChevronDown, MessageSquare, Layers, Bone, Zap, RotateCcw, AlertTriangle } from 'lucide-react';
...
export const TrackConfigModal: React.FC<TrackConfigModalProps> = ({
  ...
  // Filter Logic:
  // 1. If searching: Show matches regardless of usage (allows duplication).
  // 2. If NOT searching: Hide animations that are already in use.
  const filteredAnimations = useMemo(() => {
    return availableAnimations
      .filter(anim => {
        // EXPLICIT EXCLUSION: Setup Pose is internal and shouldn't be documented as a track animation.
  ...
  // MOVED: Conditional return must happen AFTER all hooks are called.
  if (!isOpen) return null;

  const handleResetDocs = () => {
    // Reset Tracks to initial state (5 empty tracks)
  ...
  // --- Description Updaters (Refactored to use Name) ---
  ...
  // --- Item Removers (Refactored to use Name) ---
  ...
  const handleDrop = (e: React.DragEvent, targetIndex: number) => {
    ...
    newList[targetIndex] = {
      ...newList[targetIndex],
      animations: [...newList[targetIndex].animations, newAnimationConfig]
    };
      ...
      <div className="w-full max-w-6xl h-[85vh] flex flex-col overflow-hidden border border-gray-700 rounded-xl bg-spine-dark shadow-2xl relative">
                      ...
                      >
                        ...
                        <div className="p-2 space-y-2 min-h-[80px]">
                           {track.animations.length === 0 ? (
                              ...
                              track.animations.map((anim) => (
                                 <div key={anim.id} className="flex items-center gap-3 p-3 bg-gray-900/60 border border-gray-700 rounded-lg group hover:border-gray-600 transition-colors">
        ...
        <div className="p-4 border-t border-gray-700 bg-gray-800/50 flex justify-between items-center">
           <span className="text-xs text-gray-500 hidden md:inline">
             Drag animations to tracks. Add extra documentation sections as needed.
           </span>
           
           <div className="flex items-center gap-3 ml-auto">
             <button
                onClick={() => setShowResetConfirm(true)}
                className="flex items-center gap-2 px-3 py-2 text-sm font-bold text-red-400 hover:text-red-300 border border-transparent hover:border-red-900/50 hover:bg-red-900/20 rounded transition-all mr-2"
             ...
             <button
                ...
                className="flex items-center gap-2 px-4 py-2 text-sm font-bold text-blue-200 bg-blue-900/30 border border-blue-800/50 rounded hover:bg-blue-800 hover:text-white transition-all"
             >
                <FileText size={16} />
                Export HTML Docs
             </button>
             ...
             <button 
               onClick={onClose}
               className="px-6 py-2 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded shadow-lg transition-colors"
             >
               Done
             </button>
           </div>
