...
import { SpineJson, ProcessedSkinMap, AnalysisResult, FoundImageResult, MissingImageResult, AssetUsage, AnalysisReport, GlobalAssetStat, AttachmentInfo, SpineSkin, SpineAttachmentData, UnusedAsset } from '../types';
  ...
  skinsArray.forEach(skin => {
    ...
    Object.values(skin.attachments).forEach(slotAttachments => {
      Object.values(slotAttachments).forEach(data => {
         ...
         // Clipping/BoundingBox do not have dimensions.
